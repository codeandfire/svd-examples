...
impl TextExample {
    ...
    pub fn setup(dim: usize) -> TextExample {
        ...
        for (d, doc) in docs.iter().enumerate() {
            for token in doc.split(' ') {
                ...
                count_matrix.slice_mut(s![i, d]).fill(val + 1 as usize);
            }
        }
        ...
        vocab_to_idx = vocab_to_idx
            .into_iter()
            .filter(|(_, i)| count_matrix.slice(s![i.clone(), ..]).sum() > 1)
        ...
        // take SVD
        let (u_matr, sing, vt_matr) = TruncatedSvd::new(count_matrix.clone(), TruncatedOrder::Largest)
            .decompose(dim)
            .unwrap()
            .values_vectors();
    ...
    fn display_sim_matrix(vecs: &Array<f64, Ix2>, labels: &[&str]) {
        ...
        // maximum length of the labels
        let maxlen = match labels.iter().max_by_key(|l| l.len()) {
            None => panic!("Could not find maximum."),
            Some(i) => i
        }.len();
    ...
    pub fn plot(&self, key: &str) {
        ...
        assert_eq!(vecs.len_of(Axis(1)), 2, "Vectors need to be 2-dimensional to plot!");

        let filename = format!("{}_plot.svg", key);

        // limits of x- and y-axes
        let x_lim = match vecs
            .slice(s![.., 0])
            .iter()
        ...
        let mut y_size = (x_size as f64 +
            ((y_lim/x_lim) as f64 - 1.0) * x_size as f64
        ) as u32;
        ...
        x_size = x_size + PADDING;
        y_size = y_size + PADDING;
        ...
        // fill with white background colour
        root.fill(&WHITE).unwrap();

        // function to draw a dot with a label
        let dot_and_label = |x: f32, y: f32, label: &'static str| {
            EmptyElement::at((x, y))
                + Circle::new((0, 0), 3, ShapeStyle::from(&BLACK).filled())
                + Text::new(
                    label, (5, 0), ("sans-serif", 15.0).into_font()
                )
        };
        ...
        for (row, label) in vecs.axis_iter(Axis(0)).zip(labels) {
            ...
            let (x, y) = ((&row[0]).clone() as f32, (&row[1]).clone() as f32);
            root.draw(&dot_and_label(x, y, label)).unwrap();
        }
    }
}
